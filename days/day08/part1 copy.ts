  ...
  Object.keys(mapper).forEach((key: keyof M) => {
    ...
    values.forEach((val, index) => {
      ...
      for (let i = 0; i < values.length; i++) {
        ...
        let xPos = xDiff < 0 ? val.x + xDiff : val.x - xDiff;
        let yPos = yDiff < 0 ? val.y + yDiff : val.y - yDiff;
        ...
        xPos = xDiff < 0 ? nextVal.x - xDiff : nextVal.x + xDiff;
        yPos = yDiff < 0 ? nextVal.y - yDiff : nextVal.y + yDiff;
